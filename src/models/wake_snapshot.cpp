...
{
  uint8_t * fb = screen.get_framebuffer_for_snapshot();
  size_t    sz = screen.get_framebuffer_size_for_snapshot();
  ...
  else if (fb_cache_size_ != sz) {
    // Framebuffer geometry changed mid-session (orientation flip,
    // resolution change). Drop the previous cache and re-allocate at
    // the new size; an old-size capture would fail the size check on
    // restore anyway.
    free(fb_cache_);
    #if EPUB_INKPLATE_BUILD
      fb_cache_ = (uint8_t *) heap_caps_malloc(sz, MALLOC_CAP_SPIRAM);
      ...
      fb_cache_ = (uint8_t *) malloc(sz);
    #endif
    if (fb_cache_ == nullptr) { fb_cache_size_ = 0; return false; }
    fb_cache_size_ = sz;
  }
                               ...
                               uint32_t * format_hash_out)
{
  ...
  if (fread(fb, 1, sz, f) != sz) {
    LOG_W("restore: body read failed");
    fclose(f);
    return false;
  }
  fclose(f);

  // Integrity check after read so a partial/corrupted file doesn't
  // paint garbage on the panel. Cheap (~256 KB CRC at PSRAM speeds).
  uint32_t crc = crc32(fb, sz);
  if (crc != hdr.fb_crc) {
    LOG_W("restore: CRC mismatch (0x%08x != 0x%08x), zeroing framebuffer",
          (unsigned)crc, (unsigned)hdr.fb_crc);
    memset(fb, 0xFF, sz); // 0xFF = white nibbles
    return false;
  }
  ...
  // Trigger the panel paint. screen.setup already armed s_force_full
  // and s_warm_wake_clear_pending on the warm-wake path, so this
  // single call drives the fullclear + GC16 paint in one shot.
